 ...
 * @param files form data with files to upload
 * @returns
 */
export const getPresignedUrls = async (files: ShortFileProp[]) => {
    const response = await fetch(
        "http://localhost:3000/api/files/upload/presignedUrl",
        {
            method: "POST",
            headers: {
...
export const saveFileInfoInDB = async (presignedUrls: PresignedUrlProp[]) => {
    return await fetch("http://localhost:3000/api/files/upload/saveFileInfo", {
        method: "POST",
        headers: {
            "Content-Type": "application/json",
        },
